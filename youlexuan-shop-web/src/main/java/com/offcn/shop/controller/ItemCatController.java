...
public class ItemCatController {
    ...
    public Result deleteItemCat(Long[] ids) {
        try {
            List<Long> notAllowDel = itemCatService.deleteItemCat(ids);
            if (notAllowDel.size() == 0) {
                return new Result(true, "全部删除成功");
            }
            return new Result(false, "未完全删除成功，删除失败的ID：" + notAllowDel.toString());
        } catch (Exception e) {
            e.printStackTrace();
            return new Result(false, "删除失败");
        }
