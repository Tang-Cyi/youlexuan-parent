...
public class OrderServiceImpl implements OrderService {
	...
	@Autowired
	private IdWorker idWorker;//一个分布式的ID生成工具. 保证id唯一
	/**
	 * 查询全部
	 */
	@Override
	...
	public void add(TbOrder order) {
		...
		//每一个cart 对应一个order订单 orderList对应多个订单详情。
		for (Cart cart : cartList) {
			...
			tbOrder.setStatus("1");//设置订单状态
			tbOrder.setCreateTime(new Date());//设置创建时间
			tbOrder.setUpdateTime(new Date());//设置修改时间
			tbOrder.setReceiverAreaName(order.getReceiverAreaName());//设置地址
			tbOrder.setReceiverMobile(order.getReceiverMobile());//设置收件人手机号
			tbOrder.setReceiver(order.getReceiver());//收件人姓名
			tbOrder.setSourceType(order.getSourceType());//设置订单来源
			tbOrder.setSellerId(cart.getSellerId());//设置订单来源
			...
			for (TbOrderItem orderItem : cart.getOrderItemList()) {
				orderItem.setId(idWorker.nextId());//设置订单详情主键
